...
#define DIS_GRO_HIS 10025 //群历史记录
...
void insert (int socket,char* name)
{
	ONLINE *p,*q;
...
int serach_online(char*name)
{
    ONLINE*start;
	if(head->next == NULL)
	{
		printf("head->next == NULL  serach\n");
	}
...
void delete_online(int socket)
{
    ...
    for(cur = head->next,prev =NULL;cur!=NULL&&cur->socket!=socket;prev = cur,cur = cur->next);
    if(NULL == cur)
    {
        printf("未找到\n");
        return;
...
/***********发文件*********/
void send_file(MYSQL*mysql,MASG*masg,int socket)
{
	int send_socket = serach_online(masg->recv_mem);
	masg->flag = RECV_FILE; 
	//printf("%s\n",masg->data);
	printf("masg->lenth = %d\n",masg->lenth);
	send(send_socket,masg,sizeof(MASG),0);
}
/***********退群************/
void quit_group (MYSQL*mysql,MASG*masg,int socket)
{
	MYSQL_RES *res = mysql_store_result(mysql);
	MYSQL_ROW row;
	char *sql = NULL;
	...
	printf("群表中删除..\n");
	sprintf(sql, "DELETE FROM group_list WHERE group_name ='%s' and member = '%s'", masg->group, masg->send_mem);
	if (0 != mysql_real_query(mysql, sql, strlen(sql)))
	{
		printf("query: %s\n", mysql_error(mysql));
	}
...
/**********查看群历史记录******/
void dis_group_history(MYSQL*mysql,MASG*masg,int socket)
{
	MYSQL_RES *res = mysql_store_result(mysql);
	MYSQL_ROW row;
	char *sql = NULL;
	sql = (char *)calloc(1024, sizeof(char));
	sprintf(sql, "SELECT * FROM group_message WHERE  qun = '%s' AND  recv = '%s'",masg->group,masg->send_mem );
	if (0 != mysql_real_query(mysql, sql, strlen(sql)))
	{
		printf("query: %s\n", mysql_error(mysql));
	}
...
/**********提示群未读消息*******/
void dis_group_mess_on(MYSQL *mysql, MASG *masg, int socket)
{
	MYSQL_RES *res = mysql_store_result(mysql);
	MYSQL_ROW row;
	char *sql = NULL;
	sql = (char *)calloc(1024, sizeof(char));
	sprintf(sql, "SELECT * FROM group_message  where recv ='%s'", masg->send_mem);
	if (0 != mysql_real_query(mysql, sql, strlen(sql)))
	{
		printf("query: %s\n", mysql_error(mysql));
	}
	...
	{
		printf("%s %s %s %s %s %s\n", row[0], row[1], row[2], row[3], row[4], row[5]);
		strcpy(masg->data, row[5]);
		flag = row[5][0] - '0';
		printf("flag = %d\n", flag);
		if (flag == 1 || flag == 2)
		{
			if (masg->flag == DIS_ON_MES) //显示未读群消息
			{
				strcpy(masg->group, row[3]);
				strcpy(masg->recv_mem, row[1]);
				send(socket, masg, sizeof(MASG), 0);
				printf("发送群消息提示\n");
			}
			else if (masg->flag == DIS_OFF_MES) //读取未读群消息
			{
				printf(".....\n");
				if (strcmp(masg->group, row[3]) == 0 && strcmp(masg->send_mem, row[2]) == 0)
				{
					printf("当前状态，status = %s\n", row[5]);
					strcpy(masg->recv_mem, row[1]);
					strcpy(masg->data, row[4]);
					...
					printf("未读群消息已发送!\n");
					sprintf(sql, "UPDATE group_message SET status = 0 WHERE qun ='%s' AND recv = '%s'", masg->group, masg->send_mem);
					if (0 != mysql_real_query(mysql, sql, strlen(sql)))
					{
						printf("query: %s\n", mysql_error(mysql));
					}
		...
		else if (flag == 3)
		{
			if (masg->flag == DIS_ON_MES) //显示未读群消息
			{
				...
				masg->flag = TIPS;
				send(socket, masg, sizeof(MASG), 0);
				masg->flag == DIS_ON_MES;
				printf("更新了吗?\n");
				sprintf(sql, "UPDATE group_message SET status = 0 WHERE send = '%s' AND qun = '%s'", masg->send_mem, masg->group);
				if (0 != mysql_real_query(mysql, sql, strlen(sql)))
				{
					printf("query: %s\n", mysql_error(mysql));
				}
...
void chat_group(MYSQL *mysql, MASG *masg, int socket)
{
	...
	while ((row = mysql_fetch_row(res)))
	{
		...
		if (strcmp(masg->send_mem, row[2]) == 0)
			continue;
		printf("%s 群聊发起者..\n");
		socket_send = serach_online(row[2]);
...
void invite_to_group(MYSQL *mysql, MASG *masg, int socket)
{
	...
	printf("消息已加入离线群通知!\n");
	int socket_send = serach_online(masg->recv_mem);
	if (socket_send)
	{
		send(socket_send, masg, sizeof(MASG), 0);
	}
...
void dis_fri_list(MYSQL *mysql, MASG *masg, int socket)
{
	...
	while ((row = mysql_fetch_row(res)))
	{
		printf("%s %s %s\n", row[0], row[1], row[2]);
		strcpy(masg->data, row[2]);
		status = serach_online(masg->data);
		if (status)
		{
			strcat(masg->data, "(在线)");
		}
		...
		for (i = 0; i < nfds; i++)
		{
			...
			if (events[i].events & EPOLLIN) //遇到事件！！！
			{
				...
				case DIS_FRI:
					...
					break;
				case OUT_ROOM:
					socket_send = serach_online(masg.recv_mem);
					send(socket_send, &masg, sizeof(MASG), 0);
					break;
				case A_MESS:
					//保存为普通记录
				...
				case DIS_GRO_MEM: //显示群成员
					...
					break;
				case DIS_ON_MES: //显示未读群消息
					dis_group_mess_on(&mysql, &masg, socket_c);
					break;
				case DIS_OFF_MES: //读取未读消息
					dis_group_mess_on(&mysql, &masg, socket_c);
					break;
				case DIS_GRO_HIS:
					dis_group_history(&mysql,&masg,socket_c);
					break;
				case QUIT_GRO:
					quit_group(&mysql,&masg,socket_c);
					break;
				case SEND_FILE:
					send_file(&mysql,&masg,socket_c);
					break;
				default:
					printf("没有此功能!\n");
					break;
