...
void chat_only(MYSQL*mysql,MASG*masg,int socket)
{
	...
	if(socket_send)
	{
	//	sprintf(sql, "INSERT INTO message_history (send,recv,message,status) VALUES('%s','%s','%s')",masg->send_mem,masg->recv_mem,masg->data);
	//	if (0 != mysql_real_query(&mysql, sql, strlen(sql)))
	//	{
	//		printf("query: %s\n", mysql_error(&mysql));
	//	}
	...
	while (1)
	{
		...
		for (i = 0; i < nfds; i++)
		{
			...
			if (events[i].events & EPOLLIN) //遇到事件！！！
			{
				...
				case A_MESS:
				//保存为普通记录
				sprintf(sql, "INSERT INTO message_history (send,recv,message) VALUES('%s','%s','%s')",masg.send_mem,masg.recv_mem,masg.data);
				if (0 != mysql_real_query(&mysql, sql, strlen(sql)))
				{
					printf("query: %s\n", mysql_error(&mysql));
				}
